...
public class Script_ar : MonoBehaviour
{
    ...
    public static extern void AprilTagFunctionsCombined(byte[] bitmap, int height, int width, 
        double cam_px, double cam_py, double cam_u0, double cam_v0, 
        double[] coord, double[] U, double[] V, double[] T, double[] h, double[] w, double[] apr);
    ...
    void Update()
    {
        ...
        //Height of webcamtexture plane remains fixed (10 units) but width = 10*aspect_ratio
        double x = (float)(10 * aspect_ratio) / wct.width * coord[2] - (10 * aspect_ratio) / 2;
        double y = (float)10 / wct.height * coord[0] - 10 / 2;
        ...
        float scale = (float)(10.0f / wct.height) * (float)max_dim / (float)Math.Sqrt(2);
